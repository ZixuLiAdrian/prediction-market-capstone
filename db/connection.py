...
def get_extracted_events_for_generation() -> List[ExtractedEvent]:
    """
    Retrieve extracted events that have not yet had questions generated.

    Skips any ExtractedEvent whose id already appears in candidate_questions,
    so re-running FR4 is idempotent and does not duplicate questions.
    """
    with get_cursor() as cur:
        cur.execute(
            """
...
def insert_candidate_question(question: CandidateQuestion) -> int:
    """
    Insert a single candidate question. Returns its DB-assigned id.

    Args:
        question: A validated CandidateQuestion produced by FR4.
    ...
    with get_cursor() as cur:
        cur.execute(
            ...
            INSERT INTO candidate_questions
                (extracted_event_id, question_text, category, question_type,
                 options, deadline, deadline_source, resolution_source,
                 resolution_criteria, rationale, raw_llm_response)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            RETURNING id
            """,
            (
                question.extracted_event_id,
                question.question_text,
                question.category,
                question.question_type,
                json.dumps(question.options),
                question.deadline,
                question.deadline_source,
                question.resolution_source,
                question.resolution_criteria,
                question.rationale,
                question.raw_llm_response or "",
            ),
        )
        return cur.fetchone()["id"]


def get_candidate_questions(extracted_event_id: int = None) -> List[CandidateQuestion]:
    """
    Retrieve candidate questions, optionally filtered by source extracted event.

    Args:
        extracted_event_id: If provided, return only questions from this event.
                            If None, return all questions (used by FR5/FR6).

    Returns:
        List of CandidateQuestion objects ordered by id.
    """
    with get_cursor() as cur:
