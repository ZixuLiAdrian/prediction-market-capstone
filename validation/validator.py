...
def compute_clarity_score(flags: list[str]) -> float:
    """Return a 0–1 clarity score, reducing 0.2 per validation flag (floor 0.0)."""
...
def _normalize_text(text: str) -> str:
    """Lowercase and collapse whitespace for consistent term matching."""
...
def detect_ambiguous_wording(question_text: str, resolution_criteria: str) -> bool:
    """Return True if the question text or resolution criteria contain subjective ambiguous terms."""
...
def _count_trusted_media_mentions(source: str) -> int:
    """Count how many distinct trusted media outlets are named in the source string."""
    count = 0
...
def _has_authoritative_source_anchor(source: str) -> bool:
    """Return True if the source string contains at least one known authoritative institution or URL anchor."""
...
def detect_weak_resolution_source(
    ...
    """Return True if the resolution source lacks an authoritative anchor or uses only generic fallback phrasing."""
...
def detect_weak_resolution_criteria(
    ...
    """Return True if the resolution criteria are too short, vague, or missing per-option resolution rules."""
...
def _try_parse_deadline(deadline: str) -> datetime | None:
    """Try parsing a deadline string against common date formats; return a datetime or None if unparseable."""
...
def detect_invalid_deadline_window(deadline: str) -> bool:
    """Return True if the deadline is unparseable or already in the past."""
...
def detect_unclear_binary_condition(question_text: str, question_type: str) -> bool:
    """Return True if a binary question doesn't end with '?' or lacks a clear auxiliary-verb opening."""
...
def validate_question(q: CandidateQuestion) -> ValidationResult:
    """Run all FR5 deterministic checks on a candidate question and return a ValidationResult with flags."""
    flags: list[str] = []
