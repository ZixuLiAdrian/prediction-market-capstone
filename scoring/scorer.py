...
def normalize_minmax(value: float, vmin: float, vmax: float) -> float:
    """Min-max normalize value to [0, 1]; returns 1.0 when all values are equal."""
    if vmax == vmin:
        return 1.0
...
def tokenize_to_set(text: str) -> set[str]:
    """Normalize text and return the unique word set (for Jaccard / novelty comparisons)."""
    normalized = normalize_text(text)
    return set(normalized.split()) if normalized else set()


def jaccard_similarity(a_tokens: set[str], b_tokens: set[str]) -> float:
    """Return Jaccard similarity (intersection / union) between two token sets."""
...
def compute_novelty_score(current_text: str, earlier_texts: list[str]) -> float:
    """Return 0–1 novelty score: lower when the question is lexically similar to an earlier question."""
...
def _parse_deadline(deadline: str) -> datetime | None:
    """Parse a deadline string against common date formats; return a datetime or None if unparseable."""
...
def _extract_url(text: str) -> str:
    """Extract the first HTTP/S URL from a string, stripping trailing punctuation."""
...
def is_homepage_source(resolution_source: str) -> bool:
    """Return True if the resolution source URL has no meaningful path (i.e. points to a homepage)."""
...
def is_trusted_resolution_source(resolution_source: str) -> bool:
    """Return True if the resolution source contains a known high-credibility domain."""
...
def _has_price_finance_terms(question_text: str) -> bool:
    """Return True if the question involves crypto or stock price topics (triggers domain boost)."""
...
def _is_major_sports_event(question_text: str) -> bool:
    """Return True if the question is about a major sports milestone (final, championship, world cup, etc.)."""
    text = normalize_text(question_text)
    major_terms = {
...
def _is_ordinary_match_question(question_text: str) -> bool:
    """Return True if the question is a low-value single-game or regular-season match prop."""
    text = normalize_text(question_text)
...
def is_media_event(question_text: str) -> bool:
    """Return True if the question is about media publication activity rather than real-world events."""
    text = normalize_text(question_text)
...
def is_promo_event(question_text: str) -> bool:
    """Return True if the question is about a retail sale or promotional event."""
    text = normalize_text(question_text)
    promo_terms = {
...
def is_retail_promo_event(question_text: str) -> bool:
    """Return True if the question contains retail discount or coupon language."""
    text = normalize_text(question_text)
...
def is_low_significance_event(question_text: str, category: str) -> bool:
    """Return True if the question concerns a low-stakes announcement like a blog post or app update."""
    text = normalize_text(question_text)
    low_terms = {
...
def is_weather_event(question_text: str) -> bool:
    """Return True if the question concerns weather or temperature forecasts."""
    text = normalize_text(question_text)
    weather_terms = {
...
def compute_market_interest_score(question_text: str, category: str) -> float:
    """Return a 0–1 score reflecting how likely traders are to find this question interesting."""
    text = normalize_text(question_text)
    high_terms = {
...
def compute_resolution_strength_score(resolution_source: str) -> float:
    """Return a 0–1 score for how authoritative the resolution source URL and hostname appear."""
...
def compute_time_horizon_score(deadline: str) -> float:
    """Return a 0–1 score for trading-window practicality; peaks for deadlines 3–30 days out."""
...
def is_expired_deadline(deadline: str) -> bool:
    """Return True if the deadline date has already passed (used to filter stale questions)."""
    parsed = _parse_deadline(deadline)
    if parsed is None:
        return False
...
def _soft_penalty_flags(question_text: str, category: str, resolution_source: str, prior_kept_texts: list[str]) -> list[str]:
    """Collect soft quality flags (homepage source, promo, weather, low-significance, near-duplicate)."""
    flags: list[str] = []
    if is_homepage_source(resolution_source):
...
def _compute_soft_penalty(quality_flags: list[str], question_text: str, resolution_source: str) -> float:
    """Sum penalty amounts for each soft quality flag; stacks multiplicatively for promo-type issues."""
    penalty = 0.0
...
def generate_score_explanation(
    ...
    """Return a human-readable sentence explaining why a question ranked high, middle, or low."""
...
def _build_breakdown(
    ...
    """Assemble a structured score-breakdown dict for display / storage alongside the total score."""
...
def _prepare_rows(rows: list[dict]) -> list[dict]:
    """Filter expired/media rows, attach soft penalty flags, and sort ascending by question_id."""
    filtered = [
        r for r in rows
...
def score_questions(rows: list[dict], all_question_texts_by_id: list[tuple[int, str]]) -> list[ScoredCandidate]:
    """Score and rank validated questions; return ScoredCandidate list ordered by total_score descending."""
    if not rows:
        return []
...
def score_questions_with_breakdown(
    ...
    """Score and rank questions, returning both the ScoredCandidate list and a per-question breakdown dict."""
    if not rows:
        return [], {}
...
def top_n_ranked_display_rows(scored_rows: list[dict], top_n: int = 10) -> list[dict]:
    """Slice and reshape the top-N scored rows into a flat dict format suitable for the dashboard."""
