...
def run_question_generation():
    """FR4: Generate candidate market questions from extracted events using LLM."""
    events = get_extracted_events_for_generation()
    if not events:
        logger.warning("FR4: No extracted events to generate questions for "
                       "(all already processed or none exist)")
        return
    ...
    for event in events:
        questions = generator.generate(event)
        for question in questions:
            q_id = insert_candidate_question(question)
            logger.info(
                f"Saved question {q_id} for ExtractedEvent {event.id}: "
                f"[{question.question_type}] {question.question_text[:80]}"
            )
            total_saved += 1

    logger.info(
        f"FR4 complete: {total_saved} questions generated from {len(events)} extracted events"
    )
